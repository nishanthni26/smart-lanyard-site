...
import { ArrowRight, BadgeCheck, BellRing, Bluetooth, BookOpenCheck, Building2, CalendarCheck2, Cloud, DoorOpen, GraduationCap, HeartPulse, KeyRound, LayoutDashboard, MapPin, Nfc, QrCode, ShieldCheck, ShieldPlus, Smartphone, UserRoundCheck, UsersRound, Zap } from "lucide-react";
...
const heroIdentityCardImage = "/manus-storage/smart-lanyard-alex-morgan-id-card_aa026f64.png";
...
const visualStories = [
  { image: "/manus-storage/smart-lanyard-social-post-with-logo-v2_f9cc45c6.webp", label: "THE EVERYDAY CARD", title: "ID, reimagined.", text: "One card for identity, entry, updates and safety.", alt: "Smart Lanyard identity card with QR, NFC, live updates and safety features" },
  { image: "/manus-storage/smart-lanyard-sos-safety_fbb1403f.webp", label: "SAFETY", title: "Help when it matters.", text: "A clear SOS signal can help people respond sooner.", alt: "Smart Lanyard safety card with SOS status and location update" },
  { image: "/manus-storage/smart-lanyard-qr-digital-pass_9a0c7488.webp", label: "DIGITAL PASS", title: "Scan or show.", text: "Use a QR pass when a quick visual check is helpful.", alt: "Smart Lanyard digital pass with QR code and phone scanner" },
  { image: "/manus-storage/smart-lanyard-campus-access_04227dc0.webp", label: "ACCESS", title: "One tap for entry.", text: "Bring approved places together on one lanyard.", alt: "Smart Lanyard near campus access readers" },
  { image: "/manus-storage/smart-lanyard-live-timetable_35080935.webp", label: "UPDATES", title: "The day, at a glance.", text: "Helpful timetable and class-change prompts stay close by.", alt: "Smart Lanyard displaying a live timetable and class update" },
  { image: "/manus-storage/smart-lanyard-nfc-access_2369e57d.webp", label: "NFC", title: "Tap with confidence.", text: "A familiar tap can help make access feel simple.", alt: "Smart Lanyard connecting with an NFC access reader" },
  { image: "/manus-storage/smart-lanyard-parent-connectivity_7ae1b2d9.webp", label: "COMPANION APP", title: "Stay in the loop.", text: "Relevant entry, exit and attendance updates can reach the right people.", alt: "Smart Lanyard connected to a companion app showing entry and attendance updates" },
  { image: "/manus-storage/smart-lanyard-dynamic-id_c934f7da.webp", label: "DYNAMIC ID", title: "Ready for change.", text: "The digital card can keep its information current.", alt: "Smart Lanyard showing a dynamic digital identity display" },
];
...
export default function Home() {
  ...
  return <MotionConfig reducedMotion="user"><main className="friendly-site">
    <header className="kept-nav"><button className="kept-brand" onClick={() => scrollTo("#top")} aria-label="Smart Lanyard home"><img src={brandMark} alt="" /><span>SMART<br />LANYARD</span></button><nav aria-label="Primary navigation"><button onClick={() => scrollTo("#product")}>What it is</button><button onClick={() => scrollTo("#solutions")}>Who it helps</button><button onClick={() => scrollTo("#platform")}>In action</button><a href="/faq">FAQ</a></nav><a className="kept-nav-demo" href="/demo">Request a demo <ArrowRight aria-hidden="true" /></a><ThemeToggle /></header>
    <section className="kept-hero" id="top"><div className="kept-orb kept-orb-one" aria-hidden="true" /><div className="kept-orb kept-orb-two" aria-hidden="true" /><div className="kept-orb kept-orb-three" aria-hidden="true" /><motion.div className="kept-hero-copy" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .5, ease: [0.23, 1, 0.32, 1] }}><p className="kept-eyebrow"><i /> ONE LANYARD. EVERYDAY HELP.</p><h1>Identity,<br />entry<br />and <em>safety.</em></h1><p>Smart Lanyard is a familiar ID card that helps people show who they are, enter approved places and receive useful updates.</p><div className="kept-actions"><a href="/demo" className="kept-primary">Request a demo <ArrowRight aria-hidden="true" /></a><button onClick={() => scrollTo("#product")}>See how it works <ArrowRight aria-hidden="true" /></button></div><div className="kept-proof"><span><BadgeCheck aria-hidden="true" /> Identity</span><span><DoorOpen aria-hidden="true" /> Access</span><span><CalendarCheck2 aria-hidden="true" /> Attendance</span><span><HeartPulse aria-hidden="true" /> Safety</span></div></motion.div><motion.div className="kept-hero-art" initial={{ opacity: 0, scale: .94, rotate: 3 }} animate={{ opacity: 1, scale: 1, rotate: 0 }} transition={{ type: "spring", stiffness: 120, damping: 18 }}><img className="kept-wearer" src={heroImage} alt="Person wearing a Smart Lanyard" /><motion.img className="kept-card kept-hero-identity-card" src={heroIdentityCardImage} alt="Smart Lanyard student ID card for Alex Morgan" animate={{ y: [0, -8, 0], rotate: [0, 1.25, 0, -1.1, 0] }} transition={{ duration: 4.8, repeat: Infinity, ease: "easeInOut" }} /><div className="kept-callout kept-status"><MapPin aria-hidden="true" /><span>Live status<br /><b>Ready</b></span></div><div className="kept-callout kept-entry"><Nfc aria-hidden="true" /><span>Tap to enter<br /><b>Approved</b></span></div></motion.div></section>
    ...
    <section className="friendly-visual-story" aria-labelledby="visual-story-title"><div className="friendly-visual-heading"><p className="friendly-eyebrow"><i /> SEE IT IN THE EVERYDAY</p><h2 id="visual-story-title">One lanyard.<br /><em>Many useful moments.</em></h2><p>From a simple entry tap to a safety update, the card can keep the next step close at hand.</p></div><div className="friendly-visual-grid">{visualStories.map((story, index) => <motion.article className={`friendly-visual-card${index === 0 ? " is-featured" : ""}`} key={story.title} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} whileHover={{ y: -5 }} viewport={{ once: true, amount: .15 }} transition={{ delay: Math.min(index * .045, .24), duration: .28 }}><img src={story.image} alt={story.alt} /><div><p>{story.label}</p><h3>{story.title}</h3><span>{story.text}</span></div></motion.article>)}</div></section>
    <SiteFooter />
  </main></MotionConfig>;
}
